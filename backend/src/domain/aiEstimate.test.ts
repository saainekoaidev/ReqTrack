...
describe('buildEstimatePrompt (US-036)', () => {
  it('要件と前提・出力指示を含む', () => {
...
describe('parseEstimateResponse (US-036)', () => {
  it('前後に文章があっても JSON を抽出し検証する', () => {
    const out =
      'はい、以下です。\n```json\n{"features":[{"name":"ログイン","tasks":[{"name":"設計","phase":"基本設計","estimateDays":1.5,"reason":"画面1つ"}]}]}\n```\n以上';
    const r = parseEstimateResponse(out);
    expect(r.features[0]!.name).toBe('ログイン');
    expect(r.features[0]!.tasks[0]).toMatchObject({ phase: '基本設計', estimateDays: 1.5 });
  });
