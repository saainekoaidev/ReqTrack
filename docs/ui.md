...
| S03 | ガントチャート | `/manage/gantt` | 左=WBS3階層ツリー(折り畳み, 名称/工数/稼働率/担当をインライン編集 US-046)/右=稼働時間軸の連続バー(小数日・コマ無し)。工数(人日)ヘッダ・日付は年込み・右ペイン全幅 (US-004/029/040/046) | 実装済 |
| S04 | 設定 | `/settings` | タブ(基本設定/要員/休日/参照資料/プロジェクト)。プロジェクト削除は「プロジェクト」タブ (US-022/024/030) | 実装済 |
| S05 | 進捗報告 | `/manage/reports` | 廃止(US-053)。進捗率はガントの一覧でインライン入力に統合。/manage/reports は gantt へリダイレクト | 廃止 |
...
- `/manage/*` 進捗管理系(共通シェル: ヘッダ右上に参照プロジェクト選択 `ProjectContext`、左ペインメニュー): gantt/**wbs(WBS編集, US-041)**/daily/delays(進捗報告は US-053 で廃止)。**ガント(計画済みタスク)を持つプロジェクトが無い場合は遷移不可で案内表示 (US-032)**
  - **WBS編集** (`/manage/wbs`): 実行中にタスク追加/削除/変更・工数/稼働率/担当・**前提タスク(US-050)**を編集(`WbsEditor` を新規作成と共有、任意階層 US-049、葉のみ編集)→「スケジュール再生成」で反映。再生成は 明示前提(任意のタスク間依存, トポロジカル順)＋暗黙依存(同一対象配下=工程順)＋要員(空き区間へ前詰め, 対面レビューは双方同時) を開始日起点で考慮 (US-041/045/047/050)。
