...
- `/create/*` 新規作成系(ヘッダ + 3 ステップのステッパー。作成中プロジェクトは `CreateContext` で共有):
  ...
  - **3. 見積・ガント生成** (`/create/estimate`、手組みは `/create/wbs`): ここで起点を選ぶ。**「AI で要件からガントまで生成」**(US-037, 第一選択)/「AI で見積だけ生成(後で調整)」(US-036)/「手で WBS を組む(ガントから)」。**WBS はこの選択後に生成**。レビュー自動展開・効率化調整もここ。AI は Claude Code 契約枠で実行(API 課金なし)。**ReqTrack 本体は Web アプリで VSCode 起動は不要**。
